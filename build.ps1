...
function Cmd-Publish() {
    ...
    & $dotnet publish SourceCleaner.slnx --verbosity minimal -c Release -r win-x64 /p:DebugSymbols=false
}

function Cmd-Install() {
    $path="$(Split-Path -Path $(Get-Location))\bin\SourceCleaner.exe"
    $target="$(Get-Location)\output\SourceCleaner.exe" 
